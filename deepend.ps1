...
function add-backupadmin {
    ...
    $LocalAdmin = $Computer.Create("User", "SIM-Admin")
    $LocalAdmin.SetPassword($passwd)
    $LocalAdmin.SetInfo()
    $LocalAdmin.FullName = "Sim Secured Account"
    $LocalAdmin.SetInfo()
...
Get-Service | Select StartType, Status, Name, DisplayName | Format-Table -AutoSize| Out-File -filepath "$Env:userprofile\Desktop\RS.txt"
